...
# 更新：2012-07-18
# 用意：LDA 模块
# ：param path :分词路径，流式写入分词文件，否则会直接 killed: 9，建议按月写入
...
if __name__ == "__main__":
    corpus = []
    path = r'/Users/xuanlongqin/Documents/data/covid-19/paper/Decfencioutput.txt'
    ...
    # 训练模型
    model = lda.LDA(n_topics = 3, n_iter =100, random_state = 1,alpha=0.03,eta=0.01)
    ...
    for i, word_weight in enumerate(topic_word):  
        ...
        # with open('topWords.txt','w',encoding= 'utf-8' ) as fw:
        #     fw.write(topN_index)    
    ...
    # 写入关键词
    # for k,v in c.items():
    #     with open('topicWords.txt','w',encoding= 'utf-8') as fw:
    #         fw.write(k)
    #         fw.close()
    print(c)
